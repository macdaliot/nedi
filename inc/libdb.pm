...
#---Connect as nedi db user and create tables.
	...
	print "nbrtrack\n";
	$index = ($misc::backend eq 'Pg')?'':', INDEX(device), INDEX(ifname), INDEX(neighbor), INDEX(time)';
...
sub ReadAddr{
	...
	$sth = $dbh->prepare("SELECT devices.device,mac,type FROM devices JOIN nodarp on (devip = nodip) WHERE snmpversion != 0");
	$sth->execute();
	while((my @i) = $sth->fetchrow_array){
		if( !exists $misc::ifmac{$i[1]} ){
			push @{$misc::ifmac{$i[1]}{$i[0]}},'-';
			&misc::Prt("DBG :Node MAC $i[1] belongs to $i[0] ($i[2])\n") if $main::opt{'d'};
			$nmac++;
		}
	}
	$sth->finish if $sth;
...
#TODO add: SELECT devices.device,devip,mac,ifmac FROM devices JOIN nodarp on (devip = nodip) LEFT JOIN interfaces on (ifmac = mac) WHERE ifmac is not NULL
# ifmac not null sometimes no result??? This is faster anyway: SELECT devices.device,type,devip,mac FROM devices JOIN nodarp on (devip = nodip) WHERE devip != 0
	&misc::Prt("RADR:$imac IF MAC, $nmac Node MAC, $nip IP and $nip6 IPv6 addresses read.\n");
}
...
sub ReadNbr{
	...
	my $sth = $dbh->prepare("SELECT device,ifname,neighbor FROM nbrtrack $where");
	$sth->execute();
	while((my @i) = $sth->fetchrow_array){
		if( exists $misc::portprop{$i[0]} and exists $misc::portprop{$i[0]}{$i[1]} ){
			&misc::Prt("DBG :Adding $i[2]\tto $i[0],$i[1]\n") if $main::opt{'d'} =~ /d/;
			$misc::portprop{$i[0]}{$i[1]}{ntrk}{$i[2]}++;
			$misc::portprop{$i[0]}{$i[1]}{lnk} = 'M' unless $misc::portprop{$i[0]}{$i[1]}{lnk};
			$nnbr++;
		}
	}
	$sth->finish if $sth;

	&misc::Prt("\nRNBR:$nnbr neighbors read\n");
}
			...
			foreach my $po ( keys %{$n->{$ver}{$mc}} ){
				...
				if($ver){
					...
					foreach my $ip ( keys %{$n->{$ver}{$mc}{$po}} ){
						...
						if(exists $misc::seedini{$mip} and exists $misc::seedini{$mip}{dv} and !exists $misc::ifmac{$mc}){# IP belongs to dev, but MAC is not in ifmac
							push @{$misc::ifmac{$mc}{$misc::seedini{$mip}{na}}},'-';# Adding to ifmac as macLinks rely on it
							&misc::Prt("DBG :$mc and $ip belong to $misc::seedini{$ip}{na}, adding to ifmac\n") if $main::opt{'d'};
						}
					...
					if($arpp and $nips > $arpp){					# Check for ARP poisoning on nodes
...
sub WriteDNS(){
	...
	if($dnsna and $ver){
		...
		my $dbip  = misc::IP6toDB($ip,1);							# This results in binary comparison with mysql backend...
		my $dbnam = &db::Select('dns6','nodip6','*',"nodip6=".$db::dbh->quote($dbip) );
		if( exists $dbnam->{$dbip} ){
			if( $dnsup > $misc::revive and $dbnam->{$dbip}{'dns6update'} < $misc::revive ){
		...
		my $dip   = &misc::Ip2Dec($ip);
		my $dbnam = &db::Select('dns','nodip','*',"nodip=$dip");
		if( exists $dbnam->{$dip} ){
			if( $dnsup > $misc::revive and $dbnam->{$dip}{'dnsupdate'} < $misc::revive ){
...
sub WriteLink{
	...
	$sth->execute();
	if($sth->rows){
		&misc::Prt("DBG :$dv,$i to $ne,$ni exists, updating $ty link\n") if $main::opt{'d'} =~ /d/;
...
sub Inventory{
	...
	my $dv     = $_[0];
	my $loc    = $main::dev{$dv}{lo};
	my $notnew = ($main::opt{'Y'} =~ /n/ and $main::dev{$dv}{pls})?1:0;
	if( $main::dev{$dv}{lo} =~ /$misc::locsep/ ){
		my @l = split($misc::locsep,$main::dev{$dv}{lo});
		$loc  = "$l[0]$misc::locsep$l[1]$misc::locsep$l[2]";
	}

	if( length $main::dev{$dv}{sn} > 3 ){
		if( $dbh->do("UPDATE inventory SET assetclass=3,assetupdate=$main::now,comment=".$dbh->quote("Rediscovered as $dv with IP $main::dev{$dv}{ip}").",state=100 where serial = '$main::dev{$dv}{sn}' ") + 0){
			&misc::Prt("INV :Device $main::dev{$dv}{sn} ($dv) updated in $misc::dbname.inventory\n");
		}elsif( $main::opt{'Y'} =~/[am]/ or $main::opt{'Y'} =~/s/ and $main::dev{$dv}{rv} ){
			if( $notnew ){
				&misc::Prt("INV :Only adding new devices (due to -Yn)\n");
			}elsif(length $main::dev{$dv}{sn} > 3){
				$r = $dbh->do("INSERT INTO inventory (state,serial,assetclass,assettype,assetlocation,assetcontact,assetupdate,comment) VALUES
				(100,'$main::dev{$dv}{sn}',3,'$main::dev{$dv}{ty}',".$dbh->quote($loc).",".$dbh->quote($main::dev{$dv}{co}).",".$main::now.",".$dbh->quote("Discovered as $dv with IP $main::dev{$dv}{ip}").')' );
	...
	if( $main::opt{'Y'} =~/m/ ){
		foreach my $i ( sort keys %{$main::mod{$dv}} ){
			if(length $main::mod{$dv}{$i}{sn} > 3){
				if( $dbh->do("UPDATE inventory SET assetupdate=$main::now,assetclass=$main::mod{$dv}{$i}{mc},comment=".$dbh->quote("Rediscovered in $dv $main::mod{$dv}{$i}{sl}").",state=100 where serial = '$main::mod{$dv}{$i}{sn}'") + 0){
					...
					if( $notnew ){
						...
						$r = $dbh->do("INSERT INTO inventory (state,serial,assetclass,assettype,assetlocation,assetcontact,assetupdate,comment) VALUES (100,'$main::mod{$dv}{$i}{sn}',$main::mod{$dv}{$i}{mc},'$main::mod{$dv}{$i}{mo}',".$dbh->quote($loc).",".$dbh->quote($main::dev{$dv}{co}).",".$main::now.",".$dbh->quote("Discovered in $dv $main::mod{$dv}{$i}{sl}").')' );
						&misc::Prt("INV :Module $i $main::mod{$dv}{$i}{sn} added to $misc::dbname.inventory\n") if $r;
					}
				}
			}
		}
...
sub WriteNod{
	...
	foreach my $dv ( keys %$m ){
		foreach my $mcvl ( keys %{$m->{$dv}} ){
			...
			my $vl = $m->{$dv}{$mcvl}{vl};
			$vl = 0 if $vl eq '';								# Avoid DB errors with empty vlanids
			my $if = $m->{$dv}{$mcvl}{if};
			my $me = $m->{$dv}{$mcvl}{me};
				...
				if( exists $misc::ifmac{$mc} ){
					...
					if(scalar @nbr == 1){						# Unique nbr can be used for exact MAC links
						...
						if($dv eq $nbr[0]){
							...
							if( $last = &db::Select('nbrtrack','','time',"device=".$db::dbh->quote($dv)." AND ifname='$if' AND neighbor=".$db::dbh->quote($nbr[0])) ){
								if( $last < $misc::revive){				# Only update if "halfway retired" to minimize DB load
							...
							$misc::portprop{$dv}{$if}{ntrk}{$nbr[0]}++;
						}
					}else{
						$ntxt = "Multiple-$mc!";
					}
					&misc::Prt("DBG :$mc on $if vl$vl belongs to device $ntxt\n" ) if $main::opt{'d'};
					$main::int{$dv}{$misc::portprop{$dv}{$if}{idx}}{com} .= "MAC:$ntxt,";# TODO remove in favor of nbrtrack info?
				}
