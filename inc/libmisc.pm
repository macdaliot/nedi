...
use vars qw(%oui %arp  %arp6 %arpc %arpn %portprop %portdes %vlid);
	...
	if($main::dev{$dv}{'pls'} > $main::lasdis){							# Avoid > 100% events due to offline dev being rediscovered
		...
		if($errele and $main::int{$dv}{$i}{typ} != 71 and $skip !~ /e/){			# Ignore Wlan IF
			if($main::int{$dv}{$i}{die} > $rrdstep){
				$mq += &mon::Event($errele,$lvl,$cla.'ei',$dv,$dv,"$iftxt has had many ($main::int{$dv}{$i}{die}) inbound errors in the last ${rrdstep}s!");
			...
			if($main::int{$dv}{$i}{doe} > $rrdstep){
				$mq += &mon::Event($errele,$lvl,$cla.'eo',$dv,$dv,"$iftxt has had many ($main::int{$dv}{$i}{doe}) outbound errors in the last ${rrdstep}s!");
...
sub MacLinks{
	...
	do{
		...
		foreach my $dv ( sort keys %misc::portprop ){						# Key order in Perl is random if not sorted...
			foreach my $if ( sort keys %{$misc::portprop{$dv}} ){#TODO check for existing MAC link and don't overwrite (change this letter?) add DB table to store portprop across discoveries?
				if( defined $misc::portprop{$dv}{$if}{lnk} and !$misc::portprop{$dv}{$if}{chn}){
					if($misc::portprop{$dv}{$if}{lnk} eq 'M'){
						#$ntrk = &db::Select('nbrtrack','neighbor','neighbor',"device=".$db::dbh->quote($dv)." AND ifname='$if'");
						#my @nbr = sort keys %$ntrk;
						my @nbr = sort keys %{$misc::portprop{$dv}{$if}{ntrk}};
						&misc::Prt("DEV :$curdv\t$dv\t$if\t".@nbr." neighbors\n");
						if(scalar @nbr == 1){					# A single nbr can be linked
							my $nlnk = 0;
							my $nif  = '';
							my $enif  = '';
							####print "NEB :$dv -> $nbr[0] Count $misc::portprop{$dv}{$if}{nbr}{$nbr[0]}\n";
							foreach my $cnif ( sort keys %{$misc::portprop{$nbr[0]}} ){
								if($misc::portprop{$nbr[0]}{$cnif}{lnk} eq 'M'){
									#$btrk = &db::Select('nbrtrack','neighbor','neighbor',"device=".$db::dbh->quote($nbr[0])." AND ifname='$cnif'");
									#my @bnbr = sort keys %$btrk;
							...
							if($nlnk == 1 or $enif){			# We got one or even exact backlink
								...
								&db::WriteLink(	$dv,
										...
										"Constructed (Hub)") unless $opt{'t'};# MacLinks() can be called with -t and -D...
								&db::WriteLink(	$nbr[0],
										$nif,
										$dv,
										$if,
										...
										"Constructed from $nlnk backlinks".(($enif)?" and exact interface $enif":"") ) unless $opt{'t'};
								$misc::portprop{$dv}{$if}{lnk} = 'P';	# Processed MAC-link...
								delete $misc::portprop{$nbr[0]};	# Remove linked neighbor...TODO better if not deleted?
								foreach my $dna ( keys %misc::portprop ){# ...and all its occurences
									foreach my $dpo ( keys %{$misc::portprop{$dna}} ){
										delete $misc::portprop{$dna}{$dpo}{ntrk}{$nbr[0]} if exists $misc::portprop{$dna}{$dpo}{ntrk}{$nbr[0]};
									}
								}
					...
					}elsif($misc::portprop{$dv}{$if}{lnk} eq 'D' and
						exists $misc::portprop{$misc::portprop{$dv}{$if}{nbr}} and
						exists $misc::portprop{$misc::portprop{$dv}{$if}{nbr}}{$misc::portprop{$dv}{$if}{nif}} and
						$misc::portprop{$misc::portprop{$dv}{$if}{nbr}}{$misc::portprop{$dv}{$if}{nif}}{lnk} ne 'D'){
						misc::Prt("Found halflink:$dv,$if to $misc::portprop{$dv}{$if}{nbr},$misc::portprop{$dv}{$if}{nif}\n");
						&db::WriteLink(	$misc::portprop{$dv}{$if}{nbr},
								...
								"Constructed from halflink") unless $opt{'t'};
					}
				}
			}
			$curdv++;
...
sub CheckTodo{
	...
	if($tgt =~ /^!/){
		...
		&misc::Prt("TODO:$id already processed\n");
		return 0;
	}else{
		...
		if(defined $hexip){
			...
			if( !misc::ValidIP($ip) or grep {$_ eq $ip} (@doneip,@failip) ){
				&misc::Prt("TODO:$ip unusable or already processed\n");
				return 0;
			}else{
				$seedini{$ip}{rc} = ($rc)?$rc:'';
				...
				$doip{$id} = $ip;
				push(@todo,$id);
				&Prt("TODO:Adding $ip\t($id)\t$seedini{$ip}{rc}\t$seedini{$ip}{rv}\n");
				return 1;
			}
		}else{
...
sub InitSeeds{
	...
	if( !$s and !$main::opt{'A'} and !$main::opt{'a'} ){						# Fall back to GW if no seeds were found, except if -a or -A had no results
...
sub Discover{
	...
	#if( !misc::ValidIP($doip{$id}) ){
	#	&mon::Event('d',100,'nedn',$id,'',"Unusable IP $doip{$id} using $misc::ncmd" );
	#	&misc::Prt('',"$id, IP not usable\t\t\t");
	#	$doid = 0;
	#}els
	...
	if($dv){											# Success?
		...
		db::ReadNbr('device = '.$db::dbh->quote($dv) ) if $skip !~ /F/;				# Read nbrtrack entries
...
=cut
sub ValidIP{
	if( defined $_[0] and $_[0] !~ /^[0]?$|^127\.0\.0\.|^0\.0\.0\.0$|^255\.255\.255\.255$/ ){
		return 1;
	}
}
