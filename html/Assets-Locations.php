...
if( count($in) ){
	...
	if ( $map and !isset($_GET['xls']) ){
		if($map == 3){
			echo "<p><img src=\"http://staticmap.openstreetmap.de/staticmap.php?zoom=2&size=800x500&markers=".implode('|',$omk)."\" class=\"bctr genpad\">\n";
		}elseif($map == 2){
			echo "<p><img src=\"http://maps.google.com/maps/api/staticmap?size=800x500&maptype=roadmap&sensor=false$gmk\" class=\"bctr genpad\">\n";
		}elseif( file_exists("map/map_$_SESSION[user].php") ){
			echo "<p><img src=\"map/map_$_SESSION[user].php\" class=\"bctr genpad\">\n";
		}
	}
}
