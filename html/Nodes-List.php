	...
	if($res){
		...
		while( ($n = DbFetchRow($res)) ){
			...
			if($isadmin and $mon and $n[30]){
				$mona = ($n[32])?$n[32]:$ip;
				$most = AddRecord('monitoring',"name='$mona'","name,monip,class,test,device,depend1,latwarn","'$mona','$n[30]','node','ping','".DbEscapeString($n[4])."','".DbEscapeString($n[4])."',$latw");
			}
			TblRow($bg);
			if(in_array("imBL",$col))	TblCell('','',"$bi ctr xs","+<a href=\"Nodes-Status.php?mac=$n[0]&vid=$n[21]\"><img src=\"img/oui/$img.png\" title=\"$n[1] ($n[0])\"></a>");
			...
			if( !isset($_GET['xls']) ){
				if(in_array("sshNS",$col)){
					echo "\t\t<td>\n\t\t\t<a href=ssh://$ip><img src=\"img/16/lokc.png\"></a>\n";
					echo (($wasup)?CheckTCP($ip,'22',''):"-") ."\n\t\t</td>\n";
				}
				if(in_array("telNS",$col)){
					echo "\t\t<td>\n\t\t\t<a href=telnet://$ip><img src=\"img/16/loko.png\"></a>\n";
					echo (($wasup)?CheckTCP($ip,'23',''):"-") ."</td>";
				}
				if(in_array("wwwNS",$col)){
					echo "\t\t<td>\n\t\t\t<a href=http://$ip target=window><img src=\"img/16/glob.png\"></a>\n";
					echo (($wasup)?CheckTCP($ip,'80',"GET / HTTP/1.0\r\n\r\n"):"-") ."\n\t\t</td>\n";
				}
				if(in_array("nbtNS",$col)){
					echo "\t\t<td>\n\t\t\t<img src=\"img/16/nwin.png\">\n";
					echo (($wasup)?NbtStat($ip):"-") ."\n\t\t</td>";
				}
