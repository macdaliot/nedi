	...
	if($res){
		...
		while( ($dev = DbFetchRow($res)) ){
			...
			if( $del ){
				...
				if( in_array("firstdis",$col) )	TblCell( date($_SESSION['timf'],$dev[4]),"?in[]=firstdis&op[]==&st[]=$dev[4]",'nw','',"background-color:#$fc" );
				if( in_array("lastdis",$col) )	TblCell( date($_SESSION['timf'],$dev[5]),"?in[]=lastdis&op[]==&st[]=$dev[5]",'nw','',"background-color:#$lc" );
				if(in_array("cpu",$col)){
				...
				if(in_array("devopts",$col))	TblCell($dev[27]);
				...
				if( in_array("time",$col) ){
					$cbup = ($dev[36])?"$buplbl:".date($_SESSION['timf'],$dev[36]).", $sizlbl:".DecFix($dev[34])."B".", $chglbl:".DecFix($dev[35])."B":'';
					TblCell($cbup,"Devices-Config.php?shc=$ud");
				}
				if( in_array("test",$col) ){
					if($stat){
						TblCell("$loslbl:$dev[36] OK:$dev[37]","Monitoring-Setup.php?in[]=name&op[]==&st[]=$ud",'','+'.TestImg($dev[34]) );
					}else{
						TblCell('-','','','+'.TestImg($dev[34]));
					}
				}
				if(in_array("poNS",$col)){
					$pop = NodPop( array('device'),array('='),array($dev[0]),array() );
					if($pop){
						TblCell($pop,"Nodes-List.php?in[]=device&op[]==&st[]=$ud",'',Bar($pop,100,'si').' ');
					}else{
						TblCell();
					}
				}
