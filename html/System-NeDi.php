		...
		<tr>
			<td>
				<select size="1" name="sed" onchange="UpCmd();">
					<option value="">Seed ->
					<option value="a" <?= ($sed == "a")?" selected":"" ?> ><?= (($verb1)?"$addlbl IP":"IP $addlbl") ?>
					<option value="ts -a" <?= ($sed == "ts -a")?" selected":"" ?> ><?= $tstlbl ?> IP
					<option value="A" <?= ($sed == "A")?" selected":"" ?> ><?= (($verb1)?"$addlbl DB":"DB $addlbl") ?>
				</select>
				...
				<img src="img/16/db.png" align="right" onClick="document.nedi.opt.value='all';document.nedi.sed.selectedIndex=3;UpCmd();" title="DB <?= $addlbl ?>">
			</td>
