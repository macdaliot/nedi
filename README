...
- You can use "mapna all map2IP" in nedi.conf instead of the -f option for easier handling
...
- Click on output pad in System-NeDi turns it yellow and scrolls the page automatically to the bottom (click again to revert)
...
- Numerous changes for html5 compliance (tedious, ongoing process)
...
Import it using the new Assets-Inventory module.
...
3) In System-Export click "Export" and select incidents, locations, monitoring with Destination set to 'gzip'
